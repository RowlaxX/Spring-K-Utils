...
/**
 * Extra coverage for [MutableLongObjectEntangledArray] beyond [MutableLongObjectEntangledArrayTest].
 ...
 */
class MutableLongObjectEntangledArrayCoverageTest {
    ...
    private fun <V> MutableLongObjectEntangledArray<V>.valuesInOrder(): List<V> {
    ...
    private fun field(a: MutableLongObjectEntangledArray<*>, name: String): Any? {
        val f = MutableLongObjectEntangledArray::class.java.getDeclaredField(name).apply { isAccessible = true }
        return f.get(a)
    }

    private fun capacity(a: MutableLongObjectEntangledArray<*>): Int = (field(a, "keys") as LongArray).size
    private fun startOf(a: MutableLongObjectEntangledArray<*>): Int = field(a, "start") as Int
    private fun endOf(a: MutableLongObjectEntangledArray<*>): Int = field(a, "end") as Int
    private fun keysArray(a: MutableLongObjectEntangledArray<*>): LongArray = field(a, "keys") as LongArray
    @Suppress("UNCHECKED_CAST")
    private fun valuesArray(a: MutableLongObjectEntangledArray<*>): Array<Any?> = field(a, "values") as Array<Any?>

    private fun invoke(a: MutableLongObjectEntangledArray<*>, name: String, vararg argTypes: Class<*>): java.lang.reflect.Method {
        return MutableLongObjectEntangledArray::class.java.getDeclaredMethod(name, *argTypes).apply { isAccessible = true }
    }

    private fun search(a: MutableLongObjectEntangledArray<*>, key: Long): Int =
        invoke(a, "search", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun lowerBound(a: MutableLongObjectEntangledArray<*>, key: Long): Int =
        invoke(a, "lowerBound", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun upperBound(a: MutableLongObjectEntangledArray<*>, key: Long): Int =
        invoke(a, "upperBound", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun insertAbsent(a: MutableLongObjectEntangledArray<*>, key: Long, value: Any?) {
        invoke(a, "insertAbsent", Long::class.javaPrimitiveType!!, Any::class.java).invoke(a, key, value)
    }

    private fun ensureRoom(a: MutableLongObjectEntangledArray<*>) {
        invoke(a, "ensureRoom").invoke(a)
    }

    private fun removeAt(a: MutableLongObjectEntangledArray<*>, i: Int) {
        invoke(a, "removeAt", Int::class.javaPrimitiveType!!).invoke(a, i)
    }

    private fun ascending(a: MutableLongObjectEntangledArray<*>): Boolean {
        val ks = a.let { keysInOrderRaw(it) }
        for (i in 1 until ks.size) if (ks[i] <= ks[i - 1]) return false
        return true
    }

    private fun keysInOrderRaw(a: MutableLongObjectEntangledArray<*>): List<Long> {
    ...
    fun `ensureRoom is a no-op when capacity available`() {
        val a = MutableLongObjectEntangledArray<Long>(8)
        a.put(1L, 1L)
        val cap = capacity(a)
        ensureRoom(a)
    ...
    fun `ensureRoom doubling preserves capacity via real append growth`() {
        val a = MutableLongObjectEntangledArray<Long>(2)
    ...
    fun `getOrPut hot path on present keys allocates near-zero`() {
        ...
        val window = 64
        val a = MutableLongObjectEntangledArray<Any>(128)
        ...
        // instead (see the put-get-remove test and LongObjectEntangledArrayBench).
    ...
    fun `put-get-remove sliding hot path allocates near-zero`() {
        ...
        val a = MutableLongObjectEntangledArray<Any>(64)
        val hash = HashMap<Long, Any>()
        ...
        // deopt adds allocation that the HashMap baseline absorbs identically; see LongObjectEntangledArrayBench.)
    ...
    fun `removeIf allocates near-zero per call beyond the predicate`() {
        ...
        val a = MutableLongObjectEntangledArray<Any>(256)
        ...
        // noisy to gate on (same stance as LongObjectEntangledArrayBench on full-suite contention).
    ...
    fun `pure append growth allocation is amortized and bounded`() {
        ...
        repeat(3) {
            val warm = MutableLongObjectEntangledArray<Any>(16)
            for (i in 0L until n) warm.put(i, sharedValue)
        }

        val before = bean.getThreadAllocatedBytes(tid)
        val a = MutableLongObjectEntangledArray<Any>(16)
