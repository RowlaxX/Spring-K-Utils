...
/**
 * Not a correctness test — quantifies why [MutableLongObjectEntangledArray] replaces `HashMap<Long, V>` as
 ...
 *
 * Run manually:
 *   ./gradlew test --tests "*LongObjectEntangledArrayBench" -i
 */
class MutableLongObjectEntangledArrayBench {
    ...
    fun `sliding-window churn beats HashMap on allocation and time`() {
        ...
        val loa = MutableLongObjectEntangledArray<Any>(64)
        ...
        println("=== LongObjectEntangledArray vs HashMap<Long,V> (sliding window=$window) ===")
        println("LongObjectEntangledArray  bytes/op = ${"%.2f".format(loaBytes)}  ns/op = ${"%.1f".format(loaNs)}")
