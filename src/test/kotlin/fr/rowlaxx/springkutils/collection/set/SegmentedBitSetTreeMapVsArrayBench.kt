...
/**
 * Not a correctness test — quantifies the effect of backing [fr.rowlaxx.springkutils.collection.set.SegmentedBitSet] with a
 * [fr.rowlaxx.springkutils.collection.map.MutableLongLongArrayMap] instead of a `TreeMap<Long, Long>`.
 *
 * The old design ([LegacySegmentedBitSet] below — a faithful copy of the pre-refactor `TreeMap`
 * algorithms) is measured head-to-head against the current [fr.rowlaxx.springkutils.collection.set.MutableSegmentedBitSet] on:
...
class SegmentedBitSetTreeMapVsArrayBench {
    ...
    fun `CPU build contains remove old vs new`() {
        ...
        println("=== SegmentedBitSet CPU: TreeMap (old) vs MutableLongLongArrayMap (new), $count segments ===")
    ...
    fun `RAM footprint old vs new`() {
        ...
        println("=== SegmentedBitSet RAM: TreeMap (old) vs MutableLongLongArrayMap (new), $count segments ===")
        println("old (TreeMap<Long,Long>)             = ${mb(oldMem)} MB  (${oldMem / count} B/segment)")
        println("new (MutableLongLongArrayMap)  = ${mb(newMem)} MB  (${newMem / count} B/segment)")
