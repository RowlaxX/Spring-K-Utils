...
/**
 * Not a correctness test — quantifies why [fr.rowlaxx.springkutils.collection.map.MutableLongLongArrayMap] is preferable to
 ...
 * [MutableLongObjectArrayMapBench], which only avoided key boxing.
 *
 * Run manually:
 *   ./gradlew test --tests "*MutableLongLongArrayMapBench" -i
 */
class MutableLongLongArrayMapBench {
    ...
    fun `sliding-window churn beats HashMap on allocation and time`() {
        ...
        val lla = MutableLongLongArrayMap(64)
        ...
        println("=== MutableLongLongArrayMap vs HashMap<Long,Long> (sliding window=$window) ===")
        println("LongLongArrayMap  bytes/op = ${"%.2f".format(llaBytes)}  ns/op = ${"%.1f".format(llaNs)}")
    ...
    fun `getOrDefault hot path on present keys allocates near-zero`() {
        ...
        val window = 64
        val lla = MutableLongLongArrayMap(128)
        val hash = HashMap<Long, Long>()
