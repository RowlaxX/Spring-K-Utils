...
/**
 * Functional and edge-case coverage for [fr.rowlaxx.springkutils.collection.map.MutableLongLongArrayMap], the all-primitive
 * `long -> long` twin of [fr.rowlaxx.springkutils.collection.map.MutableLongObjectArrayMap]. Mirrors that class's test suite,
 * additionally exercising the primitive-only [fr.rowlaxx.springkutils.collection.map.MutableLongLongArrayMap.getOrDefault] accessor
 * and reaching the private helpers via reflection.
 */
class MutableLongLongArrayMapTest {

    // ---- shared helpers --------------------------------------------------

    private fun MutableLongLongArrayMap.keysInOrder(): List<Long> {
        val out = ArrayList<Long>(size)
        forEach { k, _ -> out.add(k) }
        return out
    }

    private fun MutableLongLongArrayMap.valuesInOrder(): List<Long> {
        val out = ArrayList<Long>(size)
    ...
    private fun keysArray(a: MutableLongLongArrayMap): LongArray = field(a, "keys") as LongArray

    private fun invoke(a: MutableLongLongArrayMap, name: String, vararg argTypes: Class<*>): java.lang.reflect.Method =
        MutableLongLongArrayMap::class.java.getDeclaredMethod(name, *argTypes).apply { isAccessible = true }

    private fun search(a: MutableLongLongArrayMap, key: Long): Int =
        invoke(a, "search", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun lowerBound(a: MutableLongLongArrayMap, key: Long): Int =
        invoke(a, "lowerBound", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun upperBound(a: MutableLongLongArrayMap, key: Long): Int =
        invoke(a, "upperBound", Long::class.javaPrimitiveType!!).invoke(a, key) as Int

    private fun ensureRoom(a: MutableLongLongArrayMap) {
        invoke(a, "ensureRoom").invoke(a)
    }

    private fun ascending(a: MutableLongLongArrayMap): Boolean {
    ...
    fun `front eviction then append reuses freed room without losing order`() {
        val a = MutableLongLongArrayMap(4)
        var lo = 0L
        var hi = 0L
    ...
    fun `grows past the initial capacity and keeps every entry`() {
        val a = MutableLongLongArrayMap(64)
    ...
    fun `lower returns the greatest strictly-smaller value or null`() {
        ...
        assertNull(MutableLongLongArrayMap().lower(5L))
    }
    ...
    fun `sliding window churn stays correct and ascending throughout`() {
        ...
        val a = MutableLongLongArrayMap(8)
