 ...
 * values — mirroring the twin-array, `[start, end)` window design of [MutableLongObjectEntangledArray].
 * It is the all-primitive twin of that class: where [MutableLongObjectEntangledArray] stores object
 * values (like [IntIntEntangledArray] is the all-primitive twin of `IntDoubleEntangledArray`), this
 ...
 */
class MutableLongLongEntangledArray(initialCapacity: Int = 64) {
    ...
    fun copy(): MutableLongLongEntangledArray {
        val n = size
        val c = MutableLongLongEntangledArray(if (n < 1) 1 else n)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is MutableLongLongEntangledArray) return false
    ...
    companion object {
         ...
         */
        fun deserialize(bytes: ByteArray): MutableLongLongEntangledArray {
            ...
            val result = MutableLongLongEntangledArray(if (count < 1) 1 else count)
            try {
                repeat(count) {
