...
import fr.rowlaxx.springkutils.array.IntIntEntangledArray.Companion.EMPTY
import fr.rowlaxx.springkutils.array.IntIntEntangledArray.Companion.cumulativeSet
import fr.rowlaxx.springkutils.array.IntIntEntangledArray.Companion.deserialize
import fr.rowlaxx.springkutils.array.IntIntEntangledArray.Companion.weightedSum
...
/**
 * Integer-quantity twin of [IntDoubleEntangledArray]: a parallel `(level, quantity)` pair of arrays
 * with strictly ascending levels, where the quantity is an [Int] rather than a [Double]. Every
 * operation mirrors [IntDoubleEntangledArray] one-for-one — only the quantity type and its
 * multipliers ([Int]) differ.
 */
class IntIntEntangledArray {
    ...
    operator fun get(level: Int): Int {
        ...
        return if (idx < end && first[idx] == level) second[idx] else 0
    }

    private constructor(first: IntArray, second: IntArray) {
        this.first = first
        this.second = second
        this.start = 0
        this.end = first.size
    ...
    private constructor(first: IntArray, second: IntArray, start: Int, end: Int) {
        this.first = first
        this.second = second
        this.start = start
        this.end = end
    ...
    constructor(pairs: Collection<Pair<Int, Int>>, allowZero: Boolean=true) {
        val tempFirst = EntangledArrayUtils.INT(pairs.size)
        val tempSecond = EntangledArrayUtils.INT_2(pairs.size)
        var n = 0
     ...
     * `1`. Levels are unchanged and [first] is immutable, so when this vector spans the whole backing
     * array the level array is shared with the result; otherwise the `[start, end)` window is copied.
     */
    operator fun times(multiplier: Int): IntIntEntangledArray {
        if (multiplier == 0 || size == 0) return EMPTY
        if (multiplier == 1) return this

        val outQuantities = IntArray(size)
        for (c in start until end) {
            outQuantities[c - start] = second[c] * multiplier
        }
        return IntIntEntangledArray(windowLevels(), outQuantities)
    }

    /**
     * Divides every quantity by [divisor] using integer division, keeping all levels and entries.
     * Returns [EMPTY] for an empty vector and `this` unchanged for a divisor of `1`. Levels are
     * unchanged and [first] is immutable, so when this vector spans the whole backing array the level
     * array is shared with the result; otherwise the `[start, end)` window is copied.
     */
    operator fun div(divisor: Int): IntIntEntangledArray {
        if (size == 0) return EMPTY
        if (divisor == 1) return this

        val outQuantities = IntArray(size)
        for (c in start until end) {
            outQuantities[c - start] = second[c] / divisor
        }
        return IntIntEntangledArray(windowLevels(), outQuantities)
    }
     ...
     */
    fun absolute(): IntIntEntangledArray {
        if (size == 0) {
            return EMPTY
        }
        ...
        var clean = true
        for (i in start until end) {
            if (quantities[i] <= 0) { clean = false; break }
        }
        if (clean) {
            return this
        }
        ...
        val second = EntangledArrayUtils.INT_2(size)
        var out = 0

        for (i in start until end) {
            val q = abs(quantities[i])
            if (q != 0) { first[out] = levels[i]; second[out] = q; out++ }
        }
     ...
     */
    fun head(n: Int): IntIntEntangledArray {
        if (n <= 0) return EMPTY
        if (n >= size) return this
        return IntIntEntangledArray(first, second, start, start + n)
    }
     ...
     * yields [EMPTY]; an [n] at or above [size] returns `this` unchanged.
     */
    fun tail(n: Int): IntIntEntangledArray {
        if (n <= 0) return EMPTY
        if (n >= size) return this
        return IntIntEntangledArray(first, second, end - n, end)
    }
     ...
     */
    fun trim(n: Int): IntIntEntangledArray {
        if (n <= 0) return EMPTY
        if (n >= size) return this
        val newStart = start + (size - n) / 2
        return IntIntEntangledArray(first, second, newStart, newStart + n)
    }
     ...
     */
    fun dropAfter(num: Int): IntIntEntangledArray {
        val newEnd = upperBound(num)
        if (newEnd <= start) return EMPTY
        if (newEnd == end) return this
        return IntIntEntangledArray(first, second, start, newEnd)
    }
     ...
     * none is dropped.
     */
    fun dropStrictAfter(num: Int): IntIntEntangledArray {
     ...
     * window. Yields [EMPTY] when no entry qualifies and returns `this` unchanged when none is dropped.
     */
    fun dropBefore(num: Int): IntIntEntangledArray {
        val newStart = lowerBound(num)
        if (newStart >= end) return EMPTY
        if (newStart == start) return this
        return IntIntEntangledArray(first, second, newStart, end)
    }
     ...
     * none is dropped.
     */
    fun dropStrictBefore(num: Int): IntIntEntangledArray {
     ...
     * each run is written as a magic byte (the packed run-length, start-level and quantity widths),
     * the run length, its starting level, and one variable-width int per quantity — levels themselves
     * are never stored, since they are recovered as `startLevel + i`.
     */
    fun serialize(): ByteArray {
        ...
        for (p in packets) {
            ...
            val (minQ, maxQ) = ArrayIOUtils.computeMinMax(quantities, p.start, p.endExclusive)
            totalBytes += 1 +                                       // magic byte
                ArrayIOUtils.widthOf(packetSize) +
                ArrayIOUtils.widthOf(startLevel) +
                packetSize * ArrayIOUtils.widthOfRange(minQ, maxQ)  // each quantity, variable width
        }
        ...
        for (p in packets) {
            ...
            val elemWidth = ArrayIOUtils.widthOfRange(minQ, maxQ)

            buffer.put(ArrayIOUtils.packMagic(sizeWidth, startWidth, elemWidth))
            ...
            for (i in p.start until p.endExclusive) {
                ArrayIOUtils.putInt(buffer, quantities[i], elemWidth)
            }
        }
    ...
    companion object {
        val EMPTY = IntIntEntangledArray(IntArray(0), IntArray(0))
         ...
         * packet by packet, each level recovered as `startLevel + i` and each quantity read with its
         * packed width. The bytes are trusted to describe a canonical array (ascending levels, no
         ...
         */
        fun deserialize(bytes: ByteArray): IntIntEntangledArray {
            ...
            val quantities = IntArray(totalSize)
            var index = 0
            ...
            try {
                while (index < totalSize) {
                    ...
                    val elemWidth = ArrayIOUtils.widthAt(magic, 2)
                    ...
                    for (i in 0 until packetSize) {
                        ...
                        quantities[index + i] = ArrayIOUtils.readInt(buffer, elemWidth)
                    }
                    index += packetSize
                }
            } catch (e: Exception) {
            ...
            return IntIntEntangledArray(levels, quantities)
        }
        ...
        private fun result(first: IntArray, second: IntArray, out: Int): IntIntEntangledArray {
            ...
            return IntIntEntangledArray(first.copyOf(out), second.copyOf(out))
        }

        /**
         * Compacts the dense scatter buffer used by the set/overlay paths. Absent levels hold
         * [Double.NaN] (a marker no genuine integer quantity can take, since every [Int] is exactly
         * representable as a [Double]) so that a real `0` stays distinct from "no input carried this
         * level". Levels no input carried (NaN) are always skipped; a real `0` is dropped only when
         * [dropZeros]. Survivors are written into [first]/[second].
         */
        ...
        /**
         * Compacts the dense scatter buffer used by the weighted-sum paths, whose absent levels hold
         * `0` (the additive identity). Surviving non-zero levels are written back into
        ...
        private fun sort(levels: IntArray, quantities: IntArray, end: Int) {
            ...
            val scratch = EntangledArrayUtils.INT_3(end)
            for (i in 0 until end) {
        ...
        private fun fuse(levels: IntArray, quantities: IntArray, end: Int, allowZero: Boolean): Int {
            ...
            while (out < end) {
                ...
                if (!allowZero && quantities[out] == 0) break
                out++
            }
         ...
         * `0` on that side — so a level held only by [a] yields `|q_a|` and one held only by [b]
         * yields `|q_b|`. Levels whose two quantities are equal cancel to `0` and are dropped (as
         * does a `0`-against-absent entry), so the result is always canonical (ascending levels, no
         ...
         * results.
         */
        fun absoluteDiff(a: IntIntEntangledArray, b: IntIntEntangledArray): IntIntEntangledArray {
            if (a.size == 0) return b.absolute()
            if (b.size == 0) return a.absolute()

            val s = a.size + b.size
            val first = EntangledArrayUtils.INT(s)
            val second = EntangledArrayUtils.INT_2(s)
            ...
            while (i < endA && j < endB) {
                ...
                when {
                    la < lb -> {
                        ...
                        if (q != 0) { first[out] = la; second[out] = q; out++ }
                        i++
                    }
                    la > lb -> {
                        val q = abs(quantitiesB[j])
                        if (q != 0) { first[out] = lb; second[out] = q; out++ }
                        j++
                    }
                    else -> {
            ...
            while (i < endA) {
                val q = abs(quantitiesA[i])
                if (q != 0) { first[out] = levelsA[i]; second[out] = q; out++ }
                i++
            }
            while (j < endB) {
                val q = abs(quantitiesB[j])
                if (q != 0) { first[out] = levelsB[j]; second[out] = q; out++ }
                j++
            }
         ...
         *
         * By default ([dropZeros] = `true`) entries whose winning quantity is `0` are dropped, so the
         ...
         * every level that at least one input carries, including those whose winning quantity is `0`
         * (a level absent from every input is still never emitted).
         */
        fun cumulativeSet(inputs: Collection<IntIntEntangledArray>, dropZeros: Boolean = true): IntIntEntangledArray {
        ...
        private fun cumulativeSet1(array: IntIntEntangledArray, first: IntArray, second: IntArray, dropZeros: Boolean): IntIntEntangledArray {
        ...
        private fun cumulativeSet2(
            a: IntIntEntangledArray, b: IntIntEntangledArray,
            first: IntArray, second: IntArray,
        ...
        private fun denseSet2(
            ...
            val width = span + 1
            // NaN marks a level no input carries, so a later "set to 0" stays distinct from "absent".
            val scatter = EntangledArrayUtils.DOUBLE(width)
            scatter.fill(Double.NaN, 0, width)

            val levelsA = a.first
            val quantitiesA = a.second
            for (i in a.start until a.end) {
                scatter[levelsA[i] - minLevel] = quantitiesA[i].toDouble()
            }
            val levelsB = b.first
            val quantitiesB = b.second
            for (j in b.start until b.end) {
                scatter[levelsB[j] - minLevel] = quantitiesB[j].toDouble()
            }

            return collectDenseSet(first, second, scatter, minLevel, width, dropZeros)
        }

        private fun mergeSet2(
            a: IntIntEntangledArray, b: IntIntEntangledArray,
            first: IntArray, second: IntArray, dropZeros: Boolean,
        ): IntIntEntangledArray {
            val levelsA = a.first
            val quantitiesA = a.second
            val levelsB = b.first
            ...
            while (i < endA && j < endB) {
                ...
                when {
                    la < lb -> {
                        val q = quantitiesA[i]
                        if (!dropZeros || q != 0) { first[out] = la; second[out] = q; out++ }
                        i++
                    }
                    la > lb -> {
                        val q = quantitiesB[j]
                        if (!dropZeros || q != 0) { first[out] = lb; second[out] = q; out++ }
                        j++
                    }
                    else -> {
                        val q = quantitiesB[j]
            ...
            while (i < endA) {
                val q = quantitiesA[i]
                if (!dropZeros || q != 0) { first[out] = levelsA[i]; second[out] = q; out++ }
                i++
            }
            while (j < endB) {
                val q = quantitiesB[j]
                if (!dropZeros || q != 0) { first[out] = levelsB[j]; second[out] = q; out++ }
                j++
            }
        ...
        private fun cumulativeSetN(
            arrays: Array<IntIntEntangledArray>, n: Int,
            first: IntArray, second: IntArray,
            minLevel: Int, maxLevel: Int, s: Int, dropZeros: Boolean,
        ): IntIntEntangledArray {
            val span = maxLevel.toLong() - minLevel
        ...
        private fun denseSetN(
            ...
            for (k in 0 until n) {
                ...
                for (i in array.start until array.end) {
                    scatter[levels[i] - minLevel] = quantities[i].toDouble()
                }
            }
         ...
         * cached front level of each array. Runs in O(s · log N) versus the dense path's O(span).
         * Last-writer-wins is preserved by draining every head that shares the current minimum level
         * and keeping the largest array index. Requires [arrays] to be compacted (no empty entries),
         * which [cumulativeSet] ensures.
         */
        private fun mergeSetN(
            ...
            while (m > 0) {
                ...
                if (!dropZeros || bestQ != 0) { first[out] = level; second[out] = bestQ; out++ }
            }
         ...
         * Entries whose final weighted quantity is `0` are dropped, and an array whose multiplier
         * is `0` is ignored outright (it can only contribute zeros).
         */
        fun weightedSum(pairs: Collection<Pair<IntIntEntangledArray, Int>>): IntIntEntangledArray {
            val initSize = pairs.size
            val arrays = Array(initSize) { EMPTY }
            val multipliers = IntArray(initSize) { 0 }
        ...
        private fun weightedSum1(
            array: IntIntEntangledArray, multiplier: Int,
            scratchFirst: IntArray, scratchSecond: IntArray,
        ): IntIntEntangledArray {
            val srcLevels = array.first
            val srcQuantities = array.second
            var out = 0
            ...
            if (out == srcLevels.size) {
                ...
                return IntIntEntangledArray(srcLevels, scratchSecond.copyOf(out))
            }

            return IntIntEntangledArray(scratchFirst.copyOf(out), scratchSecond.copyOf(out))
        }

        private fun weightedSum2(
            a: IntIntEntangledArray, b: IntIntEntangledArray,
            multiplierA: Int, multiplierB: Int,
            first: IntArray, second: IntArray,
        ...
        private fun weightedSumN(
            arrays: Array<IntIntEntangledArray>, multipliers: IntArray,
            first: IntArray, second: IntArray,
            minLevel: Int, maxLevel: Int, s: Int,
        ): IntIntEntangledArray {
            val span = maxLevel.toLong() - minLevel
        ...
        private fun mergeSumN(
            ...
            while (true) {
                ...
                if (q != 0) { first[out] = minLevel; second[out] = q; out++ }
            }
         ...
         * with every multiplier fixed at `1`, specialized to skip the per-entry multiply and the
         * boxed `(array, multiplier)` pairs the weighted form requires. Empty arrays are ignored and a
         * level summing to `0` is dropped; a single already-canonical input is returned unchanged.
         */
        fun sum(arrays: Collection<IntIntEntangledArray>): IntIntEntangledArray {
        ...
        private fun sumN(
            arrays: Array<IntIntEntangledArray>,
            first: IntArray, second: IntArray,
            minLevel: Int, maxLevel: Int, s: Int,
        ): IntIntEntangledArray {
            val span = maxLevel.toLong() - minLevel
