...
/**
 * Represents a mutable math vector with infinite dimension.
 * Only non-zero values are stored.
 */
class MutableSparseVector(
    content: TreeMap<Int, Double> = TreeMap()
) : SparseVector(content) {

    /**
     * Sets the [value] at the specified [index].
     * If the value is 0.0, it is removed from the storage.
     */
     ...
     */
    fun setAll(range: IntRange, value: Double) {
        for (index in range) {
            set(index, value)
        }
    }
     ...
     */
    fun addAll(indexRange: IntRange, value: Double) {
     ...
     */
    fun sub(index: Int, value: Double) {
        set(index, get(index) - value)
    }
     ...
     */
    fun subAll(indexRange: IntRange, value: Double) {
        for (index in indexRange) {
            sub(index, value)
        }
    }
     ...
     */
    fun transformNonZero(action: (Int, Double) -> Double) {
        val it = content.entries.iterator()
        while (it.hasNext()) {
            val entry = it.next()
     ...
     */
    fun transform(range: IntRange, action: (Int, Double) -> Double) {
        for (index in range) {
            set(index, action(index, get(index)))
        }
    }

    fun toSparseVector(): SparseVector {
