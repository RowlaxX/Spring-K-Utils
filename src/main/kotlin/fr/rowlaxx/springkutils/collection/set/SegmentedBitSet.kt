 ...
 *
 * The segments are held in a [MutableLongLongArrayMap] mapping each segment's start to its
...
open class SegmentedBitSet internal constructor(
    internal val content: MutableLongLongArrayMap
) {
    /**
     ...
     * [TreeMap] over keep compiling; the entries are copied into the [MutableLongLongArrayMap]
     * backing store.
     */
    ...
    companion object {
        ...
        private fun fromTreeMap(map: TreeMap<Long, Long>): MutableLongLongArrayMap {
            val content = MutableLongLongArrayMap(if (map.isEmpty()) 1 else map.size)
    ...
    fun rightShifted(count: Long): SegmentedBitSet {
        ...
        val newContent = MutableLongLongArrayMap(if (content.isEmpty()) 1 else content.size)
